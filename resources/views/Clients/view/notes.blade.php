...
<table id="notesearch" class="table">
    ...
    <tbody>
        @foreach($client->notes1 as $note)

            <tr>
                <td>{{ $note->formatDateTime_created_at_iso() }}</td>
                <td><a data-toggle="modal" href="#ShowNote" data-note="{{ $note->note }}" class="button">{{ $note->getNotePlain() }}</a></td>
                <td>{{ $note->getUser() }}</td>
            </tr>
...
$(document).ready(function() {
    ...
    $( "#notes" ).children().find(".dataTables_filter").css('display', 'none');
    $( "#notes" ).children().find(".dataTables_length").css('display', 'none');
    $( "#notes" ).children().find(".dataTables_paginate").css('display', 'none');
    $( "#notes" ).children().find(".dataTables_info").css('display', 'none');
